...
export async function awardRoutes(app: FastifyInstance) {
  ...
  app.post(
    "/",
    { preHandler: [authenticate as any], preValidation: validateRequest({ body: createAwardSchema }) },
    controller.create
  );

  app.patch(
    "/:id",
    { preHandler: [authenticate as any], preValidation: validateRequest({ body: updateAwardSchema }) },
    controller.update
  );

  app.delete("/:id", { preHandler: [authenticate as any] }, controller.delete);

  app.post("/:id/image", { preHandler: [authenticate as any] }, controller.uploadImage);
}
